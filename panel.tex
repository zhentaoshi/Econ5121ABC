...
\begin{frame}{Static Linear Model}
    \begin{itemize}
        ...
        \end{align*}
    where $ \alpha_{i} = c + \alpha^*_i$ is the new intercept
...
\begin{frame}{Within-Group Transformation}
    \begin{itemize}
        ...
        \item The $N$ parameters $(\alpha_{i})_{i=1}^N$ are eliminated
    \end{itemize}
\end{frame}
...
\begin{frame}{Alternative Way}
    \begin{itemize}
        \item  lead to the same within-Group transformation
        ...
        \]
         also eliminates the fixed effects.
...
% please put the dimension to the end of the block
    \[
    \begin{bmatrix}
        \tilde{y}_1 \\
        \tilde{y}_2 \\
...
The simplified expression of data:
    \[
...
\begin{frame}{Within Estimator}
    \begin{itemize}
        ...
        \item The fixed effects (individual-specific intercept):
...
\begin{frame}{Strict Exogeneity}
    \begin{itemize}
        ...
        \item A sufficient condition: strict exogeneity:
        \[
...
\begin{frame}{ Exogeneity}
    \begin{itemize}
        ...
        $E\left(X_{it} u_{is}\right) = 0 \quad \text{for all } s \leq t \in \{1, \ldots, T\}.$
...
\begin{frame}{First Difference (FD)}
    \begin{itemize}
        ...
        \]
        \[
        y_{it-1} = \alpha_i + X_{i,t-1}^{\prime}\beta + u_{i,t-1}.
        \]
        ...
        \]
        where \(\Delta y_{it} = y_{it} - y_{i,t-1}\), and \(\Delta X_{it}\) and \(\Delta u_{it}\) are defined similarly.

        \item Collect the first-differenced variables into \(\Delta Y\) and \(\Delta X\):
        \[
...
\begin{frame}{FD Estimator}
    \begin{itemize}
        \item The first-difference estimator is:
        \[
...
\begin{frame}{Random Effect Model}
    \begin{itemize}
        ...
        \begin{align*}            
        y_{it} & = c + X_{it}^{\prime} \beta + u_{it}  \\
               & =  c + X_{it}^{\prime} \beta + \alpha_i^{\ast} + \varepsilon_{it},
        \end{align*}
        where the composite error \(u_{it} = \alpha_i^{\ast} + \varepsilon_{it}\), with 
        \(\varepsilon_{it} \sim \text{iid } (0, \sigma_{\varepsilon}^2)\),
            \(\alpha_i^{\ast} \sim \text{iid } (0, \sigma_{\alpha}^2)\),
            and uncorrelated with \(X_{it}\).
        \begin{itemize}
            
        \item \(E\left[X_{it}^{\prime} u_{it}\right] = 0\).

        \item OLS is consistent but inefficient.
...
\begin{frame}{Efficient Estimation of RE Model}
    \begin{itemize}
        ...
        \item Generalized Least Squares is the efficient estimator.

        \item Let 
        \[
        S:= E\left[u_i u_i^{\prime}\right] =
        \begin{bmatrix}
            \sigma_{\alpha}^2 + \sigma_{\varepsilon}^2 & \sigma_{\varepsilon}^2 & \cdots & \sigma_{\varepsilon}^2 \\
            \sigma_{\varepsilon}^2 & \sigma_{\alpha}^2 + \sigma_{\varepsilon}^2 & \cdots & \sigma_{\varepsilon}^2 \\
            \vdots & \vdots & \ddots & \vdots \\
            \sigma_{\varepsilon}^2 & \sigma_{\varepsilon}^2 & \cdots & \sigma_{\alpha}^2 + \sigma_{\varepsilon}^2
        \end{bmatrix}.
        \]
    \end{itemize}
\end{frame}
...
\begin{frame}{Generalized Least Squares}
    \begin{itemize}
        \item Rewrite 
        $$y_{it}  = c + X_{it}^{\prime} \beta + u_{it} = W_{it}^{\prime} \theta + u_{it}  $$
...
%     \end{assumption*}  
    ...
    \begin{itemize}
        \item Step 1: use OLS and obtain $\widehat{u}_{it}=y_{it}-W_{it} \widehat{\theta}_{OLS}$ 
        \begin{itemize}
        \item Estimate the diagonal term and the off-diagonal term in $S$ as
        \begin{eqnarray*}
            \widehat{\sigma}_{u,diag}^{2} &=& \frac{1}{nT}\sum_{i=1}^{n}\sum_{t=1}^{T}\widehat{u}_{it}^{2}  \\
            \widehat{\sigma}_{u, off}^{2} &=& \frac{1}{n}\sum_{i=1}^{n}\frac{1}{T(T-1)}\sum_{t=1}^{T} \sum_{s\neq t}\widehat{u}_{it}\widehat{u}_{is}
        \end{eqnarray*}
...
\begin{frame}{Fixed Effects vs. Random Effects}
    \begin{itemize}
        ...
        \item RE doesn't allow endogenous \(\alpha_i\).

        \item Hausman test.

        \item Data demo: \url{https://www.kaggle.com/code/jipann/panel-data-estimation}
    \end{itemize}
\end{frame}
...
\begin{frame}{Dynamic Panel Data}
    \begin{itemize}
        ...
        assume \(u_{it} \sim \text{iid } (0, \sigma^2)\) over \(i\) and \(t\),
           and \(|\beta| < 1\).
...
\begin{frame}{FE Estimator}
    \begin{itemize}
        ...
        \]
        where $\tilde{Y}_{-1}$ is the demeaned variable of the lagged dependent variable.
        
        \item It is easy to see  
        \[
        \widehat{\beta}_{FE} - \beta_0 = \frac{\tilde{Y}_{-1}^{\prime} \tilde{U}_t}{\tilde{Y}_{-1}^{\prime} \tilde{Y}_{-1}},
        \]
...
\begin{frame}{Another Angle: First Difference}
    ...
    \begin{itemize}
        ...
        \item It follows 
        \[
        \widehat{\beta}_{FD} - \beta_0 = \frac{\sum_{it} \Delta y_{i,t-1} \Delta u_{it}}{\sum_{it} \Delta y_{i,t-1}^2}
        = \frac{\sum_{it} \left(y_{i,t-1} - y_{i,t-2}\right)\left(u_{it} - u_{i,t-1}\right)}{\sum_{it} \left(y_{i,t-1} - y_{i,t-2}\right)^2}        \]
        ...
        \item $\widehat{\beta}_{FD}$ is inconsistent.
    \end{itemize}
\end{frame}
