...
% Unable to extract LaTeX: image fig/panel_data_yx.png not provided.
% Please supply the math expressions (or the image itself) to convert them accurately.
% \begin{center}
%     \includegraphics[width=0.30\linewidth]{fig/panel_data_yx.png}
% \end{center}

% \[
% y_{it} = \alpha_i + X_{it}'\beta + u_{it}, \quad i=1,\ldots,N,\; t=1,\ldots,T
% \]
\[
\underset{NT\times 1}{Y} =
\begin{bmatrix}
y_{11}\\ y_{12}\\ \vdots \\ y_{1T}\\
y_{21}\\ \vdots \\ y_{NT}
\end{bmatrix},
\quad
\underset{NT\times p}{X}
\begin{bmatrix}
X_{11}'\\ X_{12}'\\ \vdots \\ X_{1T}'\\
X_{21}'\\ \vdots \\ X_{NT}'
\end{bmatrix}
\]
\[
y_{it} = c + X_{it}'\beta + \alpha_i^* + u_{it}, \qquad \varepsilon_{it} = \alpha_i^* + u_{it}
\]
...
\[
Y = D\,\alpha + X\,\beta + u, \qquad
D := I_{N} \otimes \mathbf{1}_{T}.
\]
...
% =
% \left(
% \begin{bmatrix} D & X \end{bmatrix}' \begin{bmatrix} D & X \end{bmatrix}
% \right)^{-1}
% \begin{bmatrix} D & X \end{bmatrix}' Y
% \]

% \begin{itemize}
%     \item The slope coefficient
% \[
% \widehat{\beta}
% =
% \left(X' M_{D} X\right)^{-1} X' M_{D} Y,\]
% where $M_{D} := I_{NT} - D\left(D'D\right)^{-1}D'$.

% \item 
% The fixed effects
% \[
% \widehat{\alpha} = (D'D)^{-1} D'\!\left(Y - X\widehat{\beta}\right).
% \]
% \end{itemize}
...
\begin{frame}{Within-Group Transformation}
    \begin{itemize}
        ...
        \]
        \item Subtraction. Let  $\tilde{y}_{it}=y_{it}-\overline{y}_{i}$ be the \textbf{within-group transformation}. Define $\tilde{X}_{it}$ and $\tilde{u}_{it}$ similarly.
        \[
