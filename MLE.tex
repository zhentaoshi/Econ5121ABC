...
\bigskip
Course website: \url{https://zhentaoshi.github.io/Econ5121ABC/}}
...
\item Common in big data
\end{itemize}
\end{frame}


\begin{frame}{Likelihood Approach}


\begin{itemize}

    \item Philosophy: The most likely outcome (Abductive reasoning).
    \item Distributional assumption (instead of moment conditions)
...
\begin{itemize}
    \item (Probabilist; generative) $f(\mathbf{Y};\theta)$ is a function of $\mathbf{Y}$ given the parameter $\theta$
...
of the likelihood function in expectation.
\end{thm*}

\begin{itemize}
    \item \textbf{Kullback-Leibler information criterion }(KLIC):
\[
...
\end{frame}
%
\begin{frame}{Unbiased Estimator of Variance}
\begin{itemize}
\item $E\left(s_{N}^{2}\right)=\sigma^{2}$ is unbiased (not MLE), where
\[
...
\begin{itemize}
     ...
     \item The MLE $$\hat{\beta} = (X'X)^{-1} X'Y$$
...
Testing a hypothesis: $H_0: R \theta_0 = q$ vs. $H_1: R \theta_0 \neq q$.
    ...
    \begin{itemize}
        ...
        \item Lagrange multiplier (LM) test: constrained estimation under the null, $\ell_N (\tilde{\theta})$
        \item Likelihood ratio (LR) test: $ \ell_N (\hat{\theta}) - \ell_N (\tilde{\theta})$
    \end{itemize}
...
\begin{itemize}
    ...
    \item Also work for nonlinear hypothesis
    \item Trade-off: computational convenience vs. finite sample performance
\end{itemize}
...
\begin{itemize}
    \item Null hypothesis: $H_0: R \theta_0 = q$, where $R$ is $r \times k$ with rank $r$
    
    \item Uses the \textbf{unconstrained MLE} $\hat{\theta}$
    ...
    \item Under $H_0$:
    \[
    W \stackrel{d}{\to} \chi^2(r)
    \]
    
    \item Reject $H_0$ if $W > \chi^2_r(\alpha)$, the $(1-\alpha)$ quantile
\end{itemize}

\end{frame}


\begin{frame}{Wald Test: Intuition}

\begin{itemize}
    \item Measures how far the unrestricted estimate $\hat{\theta}$ is from satisfying the constraint
    
    \item If $H_0$ is true, $R\hat{\theta}$ should be close to $q$
    
    \item The distance $(R\hat{\theta} - q)$ is standardized by its asymptotic variance
    
    \bigskip
    
    \item \textbf{Advantages}:
    \begin{itemize}
        \item Only requires unconstrained estimation
    ...
    \begin{itemize}
        \item Not invariant to reparameterization
        \item May have poor finite sample properties
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item Also called the \textbf{Score test}
    
    \item Uses the \textbf{constrained MLE} $\tilde{\theta}$ under $H_0: R\theta = q$
    ...
    \item Under $H_0$:
    \[
    LM \stackrel{d}{\to} \chi^2(r)
    \]
    
    \item Reject $H_0$ if $LM > \chi^2_r(\alpha)$
\end{itemize}

\end{frame}


\begin{frame}{LM Test: Intuition}

\begin{itemize}
    \item If $H_0$ is true, the score at $\tilde{\theta}$ should be close to zero
    
    \item If the constraint is violated, imposing it will push us away from the true maximum, making $s_N(\tilde{\theta})$ large
    
    \bigskip
    
    \item \textbf{Advantages}:
    \begin{itemize}
        \item Only requires constrained estimation
        \item Useful when unconstrained estimation is difficult
        \item Common in misspecification tests
    \end{itemize}
    
    \item \textbf{Disadvantages}:
    \begin{itemize}
        \item Need to solve constrained optimization
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item Uses both $\hat{\theta}$ (unconstrained) and $\tilde{\theta}$ (constrained)
    ...
    \item Under $H_0$:
    \[
    LR \stackrel{d}{\to} \chi^2(r)
    \]
    
    \item Reject $H_0$ if $LR > \chi^2_r(\alpha)$
    
    \bigskip
    
    \item Measures the improvement in log-likelihood from relaxing the constraint
\end{itemize}

\end{frame}


\begin{frame}{LR Test: Intuition}

\begin{itemize}
    \item If $H_0$ is true, imposing the constraint should not significantly reduce the likelihood
    
    \item If $H_0$ is false, the constrained model fits much worse
    ...
    \begin{itemize}
        ...
        \item Often has better finite sample properties
        \item Natural interpretation as likelihood comparison
    \end{itemize}
    
    \item \textbf{Disadvantages}:
    \begin{itemize}
        \item Requires both constrained and unconstrained estimation
    \end{itemize}
\end{itemize}
