...
\begin{frame}{Conditional Expectation}
    \begin{itemize}
        ...
        \end{align*}
        due to the joint normality, where $\lambda\left( x \right) = \phi \left( x \right) / \Phi \left( x \right)$ is called the \emph{inverse Mill's ratio}. $\phi(\cdot)$ is the pdf of $N(0,1)$
        \item The regression model can be estimated by ``Heckt''.
    \end{itemize}
\end{frame}
