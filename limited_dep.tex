...
\begin{frame}{Panoramic View}
    \begin{itemize}
        ...
        \item Economists interpret the single index as a utility 
    \end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    1- X'_{i} \beta \} $ is binary. Conditional heteroskedastic.
    \item Predicted range: $E\left [y_{i}  \mid X_{i} \right] = X'_i \beta$ can go out of $\left[0,1\right]$.
    \begin{itemize}
...
A sample of $N$ observations is
    \begin{align*}
        L_N(\beta) % &= \prod_{\{i: y_i = 1\} } \Lambda \left( X'_i \beta \right)\cdot \prod_{ \{i: y_i = 0\} } \left(1 - \Lambda \left(X'_i \beta \right)\right)\\
...
\begin{frame}{Properties}
    ...
    \[\frac{\partial \ell_N \left( \beta \right)}{\partial \beta \partial \beta ^{'}} = - \sum_{i=1}^{N} \Lambda_{i}\left( 1 - \Lambda_{i} \right) X_{i} X'_i.\]
...
\begin{frame}{Distributional Assumption}
    \begin{itemize}
        ...
        \[\Pr\left( y_{i} = j \mid \mu_{i1}, \dots, \mu_{iM} \right)= \frac{\exp\left( \mu_{ij}\right)}{\sum_{k=1}^{M}\exp \left( \mu_{ik}\right)}\]
...
\begin{frame}{Normalization}
    \begin{itemize}
        ...
        \item Equivalent to $\beta_{j=1} = 0$ (including the intercept)
    ...
    where $\mu_{i1} = 0$ by standardization.
