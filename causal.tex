...
\bigskip 
    \item Hume (Skeptism): Causation as a relationship between two impressions in the mind. 
    ...
    \item Kant: Causality is not a feature of the external world, but rather a way of organizing our experience of the world. Causality is a necessary condition for human to understand the world. 
...
\textcolor{red}{how to make latex display Chinese?}
...
\begin{frame}{Job Opportunities}
    \begin{itemize}
        ...
        \item \href{https://www.bilibili.com/video/BV1VW4y1y7AR/?spm_id_from=333.337.search-card.all.click&vd_source=a4b181aa9857818286999eec4d9925e6}{Tech sector}
    \end{itemize} 
\end{frame}
...
\begin{itemize}
    ...
    \item Sample 2: $(Y_1,\ldots,X_{N_y}) \sim N(\mu_y, \sigma^2_y)$
    \item Difference in means $\Delta = \bar{X} - \bar{Y}$
    \item Hypothesis $H_0: \mu_x = \mu_y$
    \item Test statistic 
...
\item Economics
    ...
    \begin{itemize}
        ...
        \item Scale of fiscal multiplier
...
\begin{frame}{Linear Regression}
    \begin{itemize}
        ...
        \end{align*}
        under the assumption $E [ \varepsilon_i | X_i ] = 0$ implies 
        $$ \beta = \frac{E[y_i | X_i = x_1 ] - E[y_i | X_i = x_0 ]}{ x_1 - x_0}$$ for any $x_0,x_1 \in \mathcal{X}$. 
        \item The difference between two groups. 
    \end{itemize}
\end{frame}
...
\begin{frame}{Interpretation of Linear Regression: I}
    \begin{itemize}
        ...
        \item $E[y_i | X_i]$ always exists
        \item You can always run regressions, but it may not return what you want.
...
\begin{frame}{Interpretation of Linear Regression: II}

        
        Example: $y$ is salary, $X$ is college entrance.
        
        \begin{itemize}
            \item Linear regression is doable, but cannot be interpreted as ``causal''.
            \item When we worry about endogeneity, we have a causal model in mind.
            \item Causal question cannot be answered without further structures.

        \bigskip
        
            \item Impportance of the causal model: Only the causal relationship has policy implications (under partial equilibrium)
        \end{itemize}
...
\begin{frame}{Regression-based Causal Model}
    \begin{itemize}
        ...
        \item ``Keeping everything else equal, if i's $X_i$ is changed from $x_0$ to $x_1$, then $\beta$ is the average change of $y_i$.'' This is a potential outcome claim.
    
        \item Linear potential outcome
        \begin{align*}
...
\begin{frame}{CATE}
    \begin{itemize}
        ...
        \[\Delta_{i} = \left( \alpha_{1} - \alpha_{1} \right) + X'_{i} \left( \beta_{1} - \beta_{0} \right) + \left( \varepsilon_{1i} - \varepsilon_{0i}\right)\]
        ...
        \item If $\beta_1 = \beta_0$, then $ATE(X_i) = \alpha_{1} - \alpha_{0} $ is a level change.
...
\begin{frame}{Self-Selection}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item The unconfoundedness $$(\varepsilon_{i,0}, \varepsilon_{i,1}) \bot D_i \, | \, \text{Control}_i$$ does not hold in general. \bigskip
        \end{itemize}
...
\textcolor{red}{Put "Huai river" and "RDD Huai" side by side}
...
\begin{center}
    \includegraphics[width = 0.8\textwidth]{fig/diffindiff.png}
\end{center}

\textcolor{red}{insert table}
