...
\begin{frame}{Estimating ATE via Regression}
    \begin{itemize}
        \item Parametric specification with controls
        \[ y_i = \alpha + \tau D_i + X'_i \beta + \varepsilon_i \]
        \item Interpretation: Under unconfoundedness $\left( y_{1i}, y_{0i} \right) \perp D_i \mid X_i$ and overlap, $\tau$ identifies ATE.
        \item OLS on $y_i$ with regressors $(D_i, X_i)$: $\hat{\tau}$ equals the mean difference conditional on $X_i$ (Frisch–Waugh–Lovell).
        \bigskip
        \item Under randomization and overlap, $\hat{\tau}$ is unbiased and consistent for ATE
        \item Difference-in-means is less efficient. 
    \end{itemize}
\end{frame}
